...
class ProblemFinder:
    ...
    def find_problems(
        ...
        ac_problem_ids = [
            problem[self.api.get_problem_id_key()] for problem in ac_problems
        ]
        ...
        if status == "ac":
            return [
                ...
                if problem[self.api.get_problem_id_key()] in ac_problem_ids
            ]
        elif status == "not-ac":
            return [
                problem
                for problem in problems
                if problem[self.api.get_problem_id_key()] not in ac_problem_ids
            ]
        else:
            raise ValueError(
